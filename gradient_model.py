...
def make_unet_double_convolution(n_in_channels: int, n_hidden_channels: int, n_out_channels: int) -> nn.Sequential:
    return nn.Sequential(
            nn.Conv2d(n_in_channels, n_hidden_channels, kernel_size=3, padding=1, bias=True),
            nn.BatchNorm2d(n_hidden_channels),
            nn.ReLU(inplace=True),
            nn.Conv2d(n_hidden_channels, n_out_channels, kernel_size=3, padding=1, bias=True),
...
def make_unet(n_in_channels: int, n_out_channels: int) -> nn.Sequential:
    class Unet(nn.Module):
        def __init__(self) -> None:
            ...
            self.contraction_4 = make_unet_contraction(n_channels_base * 8, n_channels_base * 8)
            ...
            self.expansion_2 = make_unet_expansion(n_channels_base * 16, n_channels_base * 4)
            self.expansion_3 = make_unet_expansion(n_channels_base * 8, n_channels_base * 2)
            self.expansion_4 = make_unet_expansion(n_channels_base * 4, n_channels_base * 1)
            self.expansion_5 = make_unet_expansion(n_channels_base * 2, n_channels_base)
